...
mod tests {
    ...
    // `napi_versions` is optional in node-pre-gyp's `binary` config: it only drives the
    // `{napi_build_version}` / `{node_napi_label}` substitutions. Packages like `duckdb` ship a
    // `binary` config without it, which used to make tracing fail with a "missing field
    // napi_versions" serde error, surfacing as a fatal TurbopackInternalError during `next build`.
    //
    // Regression test for https://github.com/vercel/next.js/issues/97538
    #[test]
    fn parses_node_pre_gyp_config_without_napi_versions() {
        // Taken from `duckdb@1.4.2`'s `package.json`.
        ...
        .expect("a node-pre-gyp `binary` config without `napi_versions` should parse");
    ...
    fn parses_node_pre_gyp_config_with_napi_versions() {
        ...
        .expect("a node-pre-gyp `binary` config with `napi_versions` should parse");
