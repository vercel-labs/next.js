...
impl EcmascriptClientReferenceModule {
    ...
    async fn proxy_module(&self) -> Result<Vc<Box<dyn EcmascriptChunkPlaceable>>> {
        ...
        if let EcmascriptExports::EsmExports(exports) = &*self.client_module.get_exports().await? {
            ...
            // Instead of registering each client reference on its own, we create a client
            // module proxy and read the individual references off of it. React wraps them in
            // a proxy that throws a helpful error when a server component tries to dot into
            // a client module (e.g. namespace/compound components), instead of silently
            // resolving to `undefined`. This matches the CommonJS branch below.
            writedoc!(
                code,
                r#"
                    // This file is generated by next-core EcmascriptClientReferenceModule.
                    import {{ createClientModuleProxy }} from "react-server-dom-turbopack/server";
                    const proxy = createClientModuleProxy({server_module_path});
                "#,
            ...
            for export_name in exports.exports.keys() {
                if export_name == "default" {
                    ...
                    writedoc!(
                        code,
                        r#"
                            export const {export_name} = proxy[{export_name_str}];
                        "#,
