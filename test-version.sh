...
rm -rf .next
LOG=/workspace/.next-maintainer/reproduction-artifacts/next-server/next-$V.log
npx next build > $LOG 2>&1
nohup npx next start -p $PORT >> $LOG 2>&1 &
sleep 6
node poll.mjs http://localhost:$PORT/cached/5s 26 > /tmp/poll-$V.txt 2>&1
curl -s -o /dev/null "http://localhost:$PORT/__shutdown" 
node summarize.mjs /tmp/poll-$V.txt
